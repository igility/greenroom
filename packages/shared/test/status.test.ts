...
const delegated = { policy: 'delegated' } as const;
...
describe('canTransition', () => {
  ...
  it('lets an approver-role reviewer approve, denies a comment-only reviewer', () => {
    expect(
      canTransition('reviewer', 'in_review', 'approved', { ...disabled, reviewerRole: 'approver' }).allowed,
    ).toBe(true);
    const denied = canTransition('reviewer', 'in_review', 'approved', {
      ...disabled,
      reviewerRole: 'reviewer',
    });
    expect(denied.allowed).toBe(false);
    expect(denied.reason).toBe('REVIEWER_ROLE_FORBIDDEN');
    // A comment-only reviewer can still request changes.
    expect(
      canTransition('reviewer', 'in_review', 'changes_requested', {
        ...disabled,
        reviewerRole: 'reviewer',
      }).allowed,
    ).toBe(true);
